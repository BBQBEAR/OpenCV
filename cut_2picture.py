...
for filename in os.listdir(r"./picture/input/"):
    ...
    m7 = np.full((m7_y, m7_x, 3), (255, 255, 255), np.uint8)
    # 貼胎音/宮縮
    m6[0:mon_h, 0:m6_x] = m5[0]
    m7[0:baby_h, 0:m6_x] = m5[1]
    try:
        # 存檔 存在picture/output資料夾內
        cv2.imwrite(f"picture/output/mon_{filename}", m6, [cv2.IMWRITE_JPEG_QUALITY, 100])
        cv2.imwrite(f"picture/output/baby_{filename}", m7, [cv2.IMWRITE_JPEG_QUALITY, 100])
